...
;( function( window ) {
	...
	function BoxesFx( el, currentPage , options ) {	
		this.el = el;
	...
	BoxesFx.prototype._init = function(currentPage) {
		...
		classie.add( this.panels[currentPage], 'current' );
		...
		this.panels.forEach( function( panel ) {
			...
			for( var i = 0; i < self.panelsCount; ++i ) {
				imgReplacement += '<div class="bg-tile"><div class="bg-img"><img class="lecture-image" src="' + img.src + '" /></div></div>'
			}
	...
	BoxesFx.prototype._navigate = function( dir ) {
		...
		if(this.current%2==0){
			$(".leftUpper").toggleClass("leftUpperAnimated");
			$('.leftRightDown').toggleClass("leftRightDownAnimated");
			$('.leftLeftDown').toggleClass("leftLeftDownAnimated");
		}else{
			$(".rightUpper").toggleClass("rightUpperAnimated");
			$('.rightRightDown').toggleClass("rightRightDownAnimated");
			$('.rightLeftDown').toggleClass("rightLeftDownAnimated");
		}	
